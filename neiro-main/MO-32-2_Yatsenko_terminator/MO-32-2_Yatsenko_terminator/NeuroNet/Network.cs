...
namespace MO_32_2_Yatsenko_terminator.NeuroNet
{
    class Network
    {
        ...
        public void Train(Network net)
        {
            ...
            //foreach (var neuron in hidden_layer1.Neurons)
            //    for (int j = 0; j < neuron.Weights.Length; j++)
            //        if (neuron.Weights[j] == 0) neuron.Weights[j] = (rand.NextDouble() - 0.5) * 0.1;
            //foreach (var neuron in hidden_layer2.Neurons)
            //    for (int j = 0; j < neuron.Weights.Length; j++)
            //        if (neuron.Weights[j] == 0) neuron.Weights[j] = (rand.NextDouble() - 0.5) * 0.1;
            //foreach (var neuron in output_layer.Neurons)
            //    for (int j = 0; j < neuron.Weights.Length; j++)
            //        if (neuron.Weights[j] == 0) neuron.Weights[j] = (rand.NextDouble() - 0.5) * 0.1;
